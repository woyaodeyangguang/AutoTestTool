...
static void cmpdirgen(ASTCmpdstmt *cstmt)
{
  ...
  if(localnode) {
    List* offsets = &localsym->irdata;
    Listnode* n = offsets->head;
    printf("String ");
    /*
    if(n) {
      Initdata* idata = n->data;
      printf("OK\n");
      if(idata) {
	printf("idata, %d\n", idata);
	if(idata->ty) {
    ...
    printf("%s", localsym->symname);
    //    printf("\n%d, symclass %d %s\n", localsym->stclass, localsym->symclass, localsym->symname);
...
static void condgen(ASTExpression *expr, BBlock *tb, BBlock *fb)
{
  ...
  Type *ty;
  /*
  printf("%s", expr->kids[0].val.p);
  if(expr->op == '<' || expr->op == '>') {
    printf("%c", expr->op);
  }
  printf("%s", expr->kids[0].val);
  */
  if(constop(expr->op)){
    if(expr->val.ll)
      jumpir(tb);
...
static Symbol * funcexprgen(ASTExpression *expr)
{
  ...
  */
  ...
  //GenJava code
  //  printf("GENJAVA\n");
  /*printf("kids0 %d, %s\n", expr->kids[0]->op, expr->kids[0]->val.p);
  argb = (ASTBase*)expr->kids[1];
  while(argb) {
    IRArg* argn;
    arg = container_of(argb, ASTExpression, base);
...
static void forgen(ASTIterationstmt *stmt)
{
  ...
  printf("for(");
  if(stmt->exprstmt[0]) {
    //    printf("ITER %c, %d\n", stmt->exprstmt[0]->expr->op, stmt->exprstmt[0]->expr->kids[0]->op);
    //    strcpy(iter,stmt->exprstmt[0]->expr->kids[0]->val.p);
    //    printf("ITER %d\n", ((char*)stmt->exprstmt[0]->expr->kids[0]->val.p)[0]);
    //    printf("ITER IS %d %d\n", stmt->exprstmt[0]->expr->kids[0]->op, stmt->exprstmt[0]->expr->kids[0]->val.ll);
    //    printf("%s=%d;", iter, stmt->exprstmt[0]->expr->kids[1]->val);
    stmtirgen(&stmt->exprstmt[0]->base);
    printf(";");
  }
  //  printf("expr0 \n");
  if(stmt->exprstmt[1]) {
    //    stmtirgen(stmt->exprstmt[1]->expr);
    exprgen(stmt->exprstmt[1]->expr);
    printf(";");
    //    printf("%s%c%d;", stmt->exprstmt[1]->expr->kids[0]->val.p, stmt->exprstmt[1]->expr->op, stmt->exprstmt[1]->expr->kids[1]->val);
  }
  //  printf("expr1 \n");
  //  printf("\n");
  if(stmt->expr) {
    ...
    /*
    printf("Wnmm\n");
    printf("%s", stmt->expr->kids[0]->val.p);
    printf("OP %c\n", stmt->expr->op);
    //    if(stmt->expr->op == POST_INC_DEC_OP) 
    {
      printf("YES\n");
      printf("%c", stmt->expr->kids[1]->op);
      }*/
    //    exprgen(stmt->expr);
    //printf("end of expr\n");
  }
