...
EXPORT ASTExpression * create_postfixexpression_login( ASTBase *expr2)
{
  ASTExpression *e;
  ...
  //NEW0(e->base);
  e->base.astclass = AST_EXPRESSION;
  e->base.coord = expr2->coord;
  e->op = CALL_LOGIN; 
  e->unary = 1;
  e->kids[0] = container_of(expr2,ASTExpression,base); 

  /* Note: use this place holder for these ops */
  //  if(op==CALL_OP || op==MEMBER_OP || op==PTR_OP || op==POST_INC_DEC_OP || op==CALL_LOGIN)
    e->kids[1] = (ASTExpression*)expr2;
    //  else
    //    e->kids[1] = container_of(expr2,ASTExpression,base);
...
EXPORT void generate_expr(ASTBase* stmt)
{
  //  printf("stmt\n");
}
                  ...
                  ASTBase *exprbase, ASTBase *stmt, ASTBase *exprstmt1, ASTBase *exprstmt2)
{
  ...
  if(exprbase) {
    ...
    //    printf("Base\n");
    //    printf("for(%d=%s;\n", (char*)iterstmt->expr->op, iterstmt->expr->kids[0]->val.p);//, (char*)iterstmt->exprstmt[0]->expr->kids[0]->val.p);
    //if(iterstmt->expr->op == POST_INC_DEC_OP)
    //printf("++\n");
  ...
  if(exprstmt1) {
    ...
    //        printf("Value %d, %s\n", iterstmt->exprstmt[0]->expr->kids[0]->op, iterstmt->exprstmt[0]->expr->kids[0]->val.p);
    //printf("for(%s=%d;%s\n", (char*)iterstmt->exprstmt[0]->expr->kids[0]->val.p, iterstmt->exprstmt[0]->expr->kids[1]->val, (char*)iterstmt->exprstmt[0]->expr->kids[0]->val.p);
  }
  else {
    //printf("for(;");
  }
  if(exprstmt2){
       ...
       //printf("for(%s=%d;%s", (char*)iterstmt->exprstmt[1]->expr->kids[0]->val.p, iterstmt->exprstmt[1]->expr->kids[1]->val, (char*)iterstmt->exprstmt[1]->expr->kids[0]->val.p);

  }
  iterstmt->type = type;
  //  printf("AFTER %d\n", ((char*)iterstmt->exprstmt[0]->expr->kids[0]->val.p)[0]);
  //generate_expr(stmt);
  return iterstmt;
}
